...
const MyBuyers = () => {
    ...
    const { data, isLoading, refetch } = useQuery({
        queryKey: ["products"],
        queryFn: async () => {
            const res = await fetch(`https://machineries-sells-server-sofiuzzamansofi.vercel.app/mybuyer?email=${user?.email}`);
                    ...
                    <table className="table w-full  dark:text-black dark:bg-black">
                        ...
                        <tbody>
                            ...
                            {data && data.map((product, index) => <tr key={product._id} className="hover">
                                ...
                                <td>
                                    <p>Trx Id:  {product?._id}</p>
                                    <p>Price:  {product?.resalePrice}</p>
                                </td>
                                <td>
                                    <p>Buyer Email: {product?.buyerEmail}</p>
                                    <p>Buyer Number: {product?.buyerNumber}</p>
                                    <p>Buyer Location: {product?.buyerLocation}</p>
                                </td>
                                <td>Upcoming</td>
